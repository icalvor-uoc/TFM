...
async function getRequest(path, args) {
	var url = new URL(window.location + path)
...
var vueApp = new Vue({
  ...
  methods: {
	...
	async getExtraction(){
		...
		r = await getRequest('/filters/extraction', {selected_grefinement: this.selected_grefinement['value']})
        }
		...
		this.extraction_list = this.extraction_list.concat(r.extraction_list)
	...
	async getPreFetch(){
		...
		r = await getRequest('/filters/fetch', {selected_extraction: this.selected_extraction['value']})
        }
		
		this.selected_fetch = r.fetch_list[0]
		this.fetch_list = r.fetch_list
	...
	async getFetch(){
		...
		r = await getRequest('/filters/frefinement', {selected_fetch: this.selected_fetch['value']})
        }

		this.selected_frefinement = r.frefinement_list[0]
		this.frefinement_list = r.frefinement_list
	...
	async getNerd(){
		...
		r = await getRequest('/filters/nrefinement', {selected_nerd: this.selected_nerd['value']})
        }

		this.selected_nrefinement = r.nrefinement_list[0]
		this.nrefinement_list = r.nrefinement_list
	...
	async getTriplegen(){
		...
		r = await getRequest('/filters/tgrefinement', {selected_triplegen: this.selected_triplegen['value']})
        }

		this.selected_tgrefinement = r.tgrefinement_list[0]
		this.tgrefinement_list = r.tgrefinement_list
	...
	async getTripleval(){
		...
		r = await getRequest('/filters/terefinement', {selected_tripleval: this.selected_tripleval['value']})
        }

		this.selected_terefinement = r.terefinement_list[0]
		this.terefinement_list = r.terefinement_list
	...
	async getPreNerd() {
		...
		r = await getRequest('/filters/nerd', {selected_frefinement: this.selected_frefinement['value']})
        }

		this.selected_nerd = r.nerd_list[0]
		this.nerd_list = r.nerd_list
		await this.getNerd()
    },
	async getPreTriplegen() {
		...
		r = await getRequest('/filters/triplegen', {selected_nrefinement: this.selected_nrefinement['value']})
        }
		
		this.selected_triplegen = r.triplegen_list[0]
		this.triplegen_list = r.triplegen_list
	...
	async getPreTripleval() {
		...
		this.selected_tripleval = r.tripleval_list[0]
		this.tripleval_list = r.tripleval_list
	...
	async getFRefinement(){
		...
		r = await getRequest('/read/frefinement', {selected_frefinement: this.selected_frefinement['value']})
		}
	...
	async getNRefinement(){
		...
		r = await getRequest('/read/nrefinement', {selected_nrefinement: this.selected_nrefinement['value']})
        }
	...
	async getTGRefinement(){
		...
		r = await getRequest('/read/tgrefinement', {selected_tgrefinement: this.selected_tgrefinement['value']})
        }
